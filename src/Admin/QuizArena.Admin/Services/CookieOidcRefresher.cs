...
/// <summary>
/// Non-interactive access_token refresh (official BlazorWebAppOidcBffAutoYarpAspire sample pattern).
/// On cookie validation, if the access_token is near expiry, exchanges the refresh_token at the
/// OIDC token endpoint and reissues the cookie with fresh tokens. If the refresh fails the
/// principal is rejected (sign-out) and the user must re-authenticate.
/// </summary>
internal sealed class CookieOidcRefresher(IOptionsMonitor<OpenIdConnectOptions> oidcOptionsMonitor)
{
    ...
    {
        // The original nonce cookie is deleted at the end of the authorization code flow;
        // refresh requests are not intended to carry a nonce.
        RequireNonce = false,
    };
