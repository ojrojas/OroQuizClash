...
public sealed record ArchiveQuestionCommand(Guid Id) : ICommand<Result<CreateQuestionResponse>>;

public sealed class ArchiveQuestionHandler(
    IRepository<Question, QuestionId> repository,
    IUnitOfWork unitOfWork) : ICommandHandler<ArchiveQuestionCommand, Result<CreateQuestionResponse>>
{
    public async Task<Result<CreateQuestionResponse>> HandleAsync(ArchiveQuestionCommand command, CancellationToken ct)
    {
...
public sealed class ArchiveQuestionEndpoint : IEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapPost("/api/questions/{id:guid}/archive", async (Guid id, ISender sender, CancellationToken ct) =>
        {
            var result = await sender.SendAsync(new ArchiveQuestionCommand(id), ct);
