...
## Player Rounds (030)
`src/app/features/game/ladder.model.ts` + `player-rounds.store.ts` + `player-rounds.component.ts/.css` — Ladder vertical Round 1..N (`PlayerRoundsStore` `signalStore` `LadderState {ladder:LadderRow[], maxRounds, currentRoundNumber, secured, rewardRules, status, _animatingRound, previousRoundNumber}` `computed currentLevel/previousLevels/nextReward/securedReward/finalRow/announcement` `hydrateLadder rxMethod GET /players/me → buildLadder(maxRounds, rounds, rewardRules, secured, current, pointsPerRound)` + `bindRealtimeLadder debounce 100ms` → hydrate, `buildLadder` pure `1..N` `state completed|current|upcoming` `isFinal n===max` `isSecured <=checkpoint` `currentReward/nextRewardFlag/securedFlag` placeholder "—", `GameComponent` `grid 280px 1fr ≥1024px sidebar sticky else flex column 375px` embeds `<app-player-rounds [gameId]>` `role="list"` `aria-current="step"` escudo `Asegurado` + corona `Final` `aria-live="polite"` transition `<400ms scale 1.02 var(--shadow-premium)` + `@media prefers-reduced-motion` none, `data-theme="player"` tokens `var(--space-*/--color-*/--player-gradient-final)` responsive `max-height:60vh overflow-y:auto` 44px WCAG AA axe, sync server only after hydrate (V).
